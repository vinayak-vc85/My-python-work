...
layout = [
    [sg.Text('Note:')],
    [sg.Text('The  column Names in the excel file should not contains space ')],
    [sg.Text('Provide Unique column as the key column')],
    [sg.Text('If there is no single unique column, \n kindly choose the multiple columns for being unique')],
    [sg.Button("OK"), sg.Cancel()]
]
  
window = sg.Window('Highbrow Digilence Services', layout)
...
"""
list2=["Name","Age","class","Dept"]
for val in user_list:
    if val in list2:
        pass
    else:
        print("[Warning:] The column named {}, that you entered is not in the column list. kindly check the column name".format(val))
"""


writer = pd.ExcelWriter("data_difference.xlsx", engine='xlsxwriter')

def check_columns(list1,list2,file_val):
    for val in list1:
        if val in list2:
            pass
        else:
            print("[Warning:] The column named {}, that you entered is not in the column list of {}. kindly check the column name".format(val,file_val))


def write_excel(DataFrame,sheetname):
...
path_OLD=filedialog.askopenfilename(initialdir = "/", title = "Select first excel File to Compare",  filetypes = (("Excel files", "*.xlsx*"),("all files","*.*")))
path_NEW=filedialog.askopenfilename(initialdir = "/", title = "Select Second excel File to compare",  filetypes = (("Excel files", "*.xlsx*"),("all files","*.*")))

#path_OLD="D:/demo.xlsx"
#path_NEW="D:/demo_output.xlsx"
# Read in both excel files
df_left_ns  = pd.read_excel(path_OLD,  na_values='None')
df_right_ns  = pd.read_excel(path_NEW,  na_values='None')
...
check_columns(user_list,list(df_left.columns),path_OLD)
check_columns(user_list,list(df_right.columns),path_NEW)
...
write_excel(df_left_join,"Left file only")
...
write_excel(new_df_right,"Right file only")
...
try:
    write_excel(df_all_diffference,"All Difference")  
except:
    write_excel(df_all_diffference_exception,"All Difference")  

print("Both Matches")
print(df_all_matches)
write_excel(df_all_matches,"Both Matches") 
writer.save()       
